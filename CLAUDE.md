...
- **OpenAI 호출 위치**: `app/api/` 하위 — `fortune`, `physiognomy`, `palmistry`, `name`, `match`, `dream`, `mbti`, `cron` 라우트. 그리고 [`lib/saju/vip-ai.ts`](lib/saju/vip-ai.ts)(VIP 리포트, Gemini 실패 시 폴백). 기본 모델 `gpt-4o-mini`. Gemini는 [`app/api/saju/vip/route.ts`](app/api/saju/vip/route.ts) + `vip-ai.ts`에서 사용.
...
- **API 라우트 목록 — 외부(miracle-toss·명운-rn)가 호출하므로 변경 금지 대상** (경로·요청/응답 형식 유지, 필요 시 새 라우트만 추가)
  ...
  | `/api/saju/vip` | POST | VIP 대운 14p 리포트 생성(Gemini, `maxDuration=300`) |
...
- **환경변수 목록과 역할** (이름만 — 값은 절대 기록 금지, `.env.local`은 git-ignore)
  ...
  - `GEMINI_API_KEY` — Google Gemini(VIP 리포트 주력).
  ...
  - `VIP_REPORT_OPENAI_MODEL` — (선택) VIP 리포트 OpenAI 모델 오버라이드.
